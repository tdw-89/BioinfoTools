...
# Returns
- `Vector{Any}`: A two-element vector containing [attribute_names, attribute_dict]
"""
...
Add promoter regions to genomic features.
...
"""
    addsequences!(ref_genome, sequence_files; feature_type="scaffold")

Add genomic sequences to features in the reference genome.

Note: This function is currently incomplete.

# Arguments
- `ref_genome::RefGenome`: Reference genome to add sequences to
- `sequence_files::Union{String, Vector{String}}`: Path(s) to sequence files
- `feature_type::String="scaffold"`: Type of feature to add sequences to
